...
NOTE: Due to file size constraints on GitHub, raw model outputs (referenced in the R script as "...model.out.RData" for the MainModel, PriorPredCheck, Simulations, and SensitivityAnalysis) could not be uploaded. These raw outputs contained the 6000 posterior samples generated for each parameter in the model. The absence of these raw outputs should not be a problem because they were only necessary for computing occupancy and occupancy change, which are summarized in the uploaded objects OccupancySummary.RData and delta.occSummary.RData, respectively. However, if you would like a copy of the raw outputs for the main analysis, you can download the folder at https://www.dropbox.com/scl/fo/6ik0hmv6u4hxvyq5vlas5/ALvRm533T63VD08Liq5kwVk?rlkey=ashv3fmtfyw7vfb6wrgukzvng&st=hrtrbmek&dl=0 , which is a duplicate of the MainModel folder here on GitHub but it contains an extra file: the raw model.out.RData. If you would like a copy of the raw model outputs from the Prior Predictive Check, Simulations, or Sensitivity Analysis, please contact me via email.
