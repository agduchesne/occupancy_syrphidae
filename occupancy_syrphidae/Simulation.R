...
# If you are only looking to graph existing simulation data, skip to "Plotting Modelled Occupancy Against Known Occupancy" section around line 520
...
#Create visit history V (whether the site was visited in this site*era*interval)
for (site in 1:sim.nsite){
  for (era in 1:4){
    for (interval in 1:6){
...
#save(sim.OccupancySummary,file=paste("Simulations/Sim",SimulationNumber,"/sim.OccupancySummary.RData",sep=""))
# Load Occupancy Summary
load(paste("Simulations/Sim",SimulationNumber,"/sim.OccupancySummary.RData",sep=""))
...
#save(sim.delta.occSummary,file=paste("Simulations/Sim",SimulationNumber,"/sim.delta.occSummary.RData",sep=""))
# Load delta Occupancy Summary
load(paste("Simulations/Sim",SimulationNumber,"/sim.delta.occSummary.RData",sep=""))
...
SimulationNumber <- 3 #Choose a simulation number to plot
load(paste("Simulations/Sim",SimulationNumber,"/sim.ModelSummary.RData",sep=""))
load(paste("Simulations/Sim",SimulationNumber,"/sim.params.RData",sep=""))
load(paste("Simulations/Sim",SimulationNumber,"/sim.relevant.dat.RData",sep=""))
load(paste("Simulations/Sim",SimulationNumber,"/sim.OccupancySummary.RData",sep=""))
...
caterpillar_plot_sim.occ.all <- function(){
  occ.to.plot <- sim.OccupancySummary %>% arrange(Mean) #Sort by ascending Mean
  occ.to.plot$Label <- paste(occ.to.plot$Species,"-",occ.to.plot$Era,sep="")
  occ.to.plot$Label <- factor(occ.to.plot$Label, levels=unique(occ.to.plot$Label),ordered=T) #Do this to prevent ggplot from automatically sorting the species on the x axis alphabetically
  #Create caterpillar plot using ggplot2
...
caterpillar_plot_sim.delta.occ.all <- function(era1,era2){
  if(era1 > era2){tmp <- era1; era1 <- era2; era2 <- tmp}
  true.delta.occ <- as.vector(sim.compute.true.delta.occ(era1,era2))
  delta.occ.to.plot <- filter(sim.delta.occSummary,EraContrast==paste(era2,"-",era1,sep=""))
  delta.occ.to.plot <- mutate(delta.occ.to.plot, true.delta.occ=true.delta.occ) %>% arrange(Mean) #Add column with true delta occ values then sort by ascending mean
...
load(paste("Simulations/Sim",SimulationNumber,"/sim.OccupancySummary.RData",sep=""))
load(paste("Simulations/Sim",SimulationNumber,"/sim.delta.occSummary.RData",sep=""))
occ1 <- caterpillar_plot_sim.occ.all()
recent1 <- caterpillar_plot_sim.delta.occ.all(3,4)
historic1 <- caterpillar_plot_sim.delta.occ.all(1,4)
...
load(paste("Simulations/Sim",SimulationNumber,"/sim.OccupancySummary.RData",sep=""))
load(paste("Simulations/Sim",SimulationNumber,"/sim.delta.occSummary.RData",sep=""))
occ2 <- caterpillar_plot_sim.occ.all()
recent2 <- caterpillar_plot_sim.delta.occ.all(3,4)
historic2 <- caterpillar_plot_sim.delta.occ.all(1,4)
...
load(paste("Simulations/Sim",SimulationNumber,"/sim.OccupancySummary.RData",sep=""))
load(paste("Simulations/Sim",SimulationNumber,"/sim.delta.occSummary.RData",sep=""))
occ3 <- caterpillar_plot_sim.occ.all()
recent3 <- caterpillar_plot_sim.delta.occ.all(3,4)
historic3 <- caterpillar_plot_sim.delta.occ.all(1,4)
