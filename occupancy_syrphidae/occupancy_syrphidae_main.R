...
p1 <- ggplot(data=SyrphidDataset, aes(x=Year)) + 
  ...
  theme(axis.text.x=element_text(angle=0, vjust=1, hjust=0.38), panel.background=element_blank(),axis.line=element_line(color="black"),panel.grid.major.x = element_blank(),axis.title = element_text(size = 20), axis.text=element_text(size=18), plot.margin = margin(10, 20, 10, 10)) + 
...
# Recent and historic caterpillar plots of delta occupancy of all species
# Recent
caterpillar_plot_delta.occ <- function(era1,era2){
  ...
  ggplot(delta.occ.to.plot[-which(delta.occ.to.plot$Species=="cross.sp.mean"),], aes(x = Species, y = Mean)) + #do not plot the cross.sp.mean as a vertical bar (take it out of the dataset)
    ...
    theme(panel.grid.major.y = element_blank(),axis.ticks.y = element_line(color = "black"), panel.grid.major.x=element_blank(), axis.text.x=element_blank(), axis.text.y = element_text(size = 22), plot.title = element_text(face = "bold"), axis.title.y = element_text(margin = margin(r = 10), size=22), axis.title.x = element_text(size=22))
}
caterpillar_plot_delta.occ(3,4)
#Historical
caterpillar_plot_delta.occ <- function(era1,era2){
  contrast.to.plot <- ifelse(era1 > era2, paste(era1,"-",era2,sep=""), paste(era2,"-",era1,sep="")) #Find the name of the contrast to plot. Make sure it is in the format [larger era]-[lesser era]
  delta.occ.to.plot <- filter(delta.occSummary, EraContrast==contrast.to.plot) %>% arrange(Mean) #Filter the delta.occupancy values to only the desired era contrast. Sort by ascending Mean
...
Species_table <- right_join(BodySize_table, Species_table, by="genusspecies") %>% relocate("BodySize",.after="RangeSize") #add bodysize column to species table
...
### Results Table showing each species and their recent and historical estimates (For table of estimates in Supplementary Material) ###
Results.supplement <- Species_table[,c("genusspecies","RecentEstimate","HistoricEstimate","RecentTrend","HistoricTrend")] %>%
...
### Sensitivity Analysis ###
# Total Number of Species in Decline and Increase
grid.arrange(plot.Sensitivity.TotalDeclinesIncreases("Spatial"),plot.Sensitivity.TotalDeclinesIncreases("Temporal (era)"),plot.Sensitivity.TotalDeclinesIncreases("Temporal (interval)"),ncol=3)
# Estimates of Across-Species Occupancy Change
grid.arrange(plot.Sensitivity.DeltaOcc("Spatial"),plot.Sensitivity.DeltaOcc("Temporal (era)"),plot.Sensitivity.DeltaOcc("Temporal (interval)"),ncol=3)


### All Pairwise Contrasts Between Eras ###
...
# Box plot of occupancy in each era across all species
boxplot_all.sp.occ <- function(){
  ...
  boxplot(boxplot.occ.data,names=EraLabels,pch=1,cex=0.5)
  mtext("Occupancy estimates in each era across all species", line=2, outer=F, cex=1) #add an overall title
  mtext("Occupancy", side=2, outer=T)
  mtext("Era", line=0 ,side=1, outer=T)
}
boxplot_all.sp.occ()


### Results Tables for Association Analyses ###
# Plot of RangeSize for recent and historic
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
par(mfrow=c(1,2))
...
# Plot of BodySize for recent and historic
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
par(mfrow=c(1,2))
...
grid.arrange(grid.grabExpr(mosaic(X2.larval.recent, shade=T, legend=F, rot_labels=c(90,90,0,0), just_labels=c("left","right"), offset_varnames = c(3.5,6), margins = c(4, 15, 4, 4), set_varnames=c(LarvalGuild="Larval feeding guild",RecentTrend="Recent Trend (between 1961\u20131990 and 1991\u20132020)"),gp_labels = gpar(fontsize = 15),gp_varnames = gpar(fontsize = 16, fontface=2))), 
...
### Strongly Supported Declines and Increases ###
