# MAIN SCRIPT
# From https://github.com/agduchesne/occupancy_syrphidae
# Please set working directory to occupancy_syrphidae folder and make sure to unzip the Datasets folder
# Note: if you wish to download and use the raw model outputs (which could not be put on GitHub due to file size), replace the MainModel folder in occupancy_syrphidae with the MainModel folder found at https://www.dropbox.com/scl/fo/6ik0hmv6u4hxvyq5vlas5/ALvRm533T63VD08Liq5kwVk?rlkey=ashv3fmtfyw7vfb6wrgukzvng&st=hrtrbmek&dl=0
...
findIDyear <- function(text) {
  if (grepl("'\\d{2}|19\\d{2}|20\\d{2}",text)==TRUE){ #\\d specifies any digit {2} exactly 2 times. Search for any 2 digits that follow either an apostrophe (e.g., '82, '11,), a 19, or a 20.
...
# Remove outlier records that are geographically anomalous based on range plots (which were plotted by the next section of code: "Preparing Occupancy Data")
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Dioprosopa (Pseudodoros) clavatus" & Latitude==43.25812 & Longitude==-81.81577))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Dasysyrphus pacificus" & Latitude==68.66898 & Longitude==-134.0716))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Sericomyia chalcopyga" & Latitude==39.35000 & Longitude==-93.9900))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Brachypalpus oarus (margaritus)" & Latitude==49.18333 & Longitude==-119.55000))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Cheilosia prima" & Latitude==53.95000 & Longitude==-114.10000))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Chrysogaster antitheus (ontario)" & Latitude==52.39700 & Longitude==-116.07700))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Chrysotoxum chinook" & Latitude==42.02286 & Longitude==-93.62679))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Copestylum avidum" & Latitude==43.41528 & Longitude==-79.76806))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Criorhina nigripes" & Latitude==44.35000 & Longitude==-68.2820))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Criorhina tricolor" & Latitude==39.70289 & Longitude==-75.11184))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Eupeodes luniger (vockerothi)" & Latitude==34.10000 & Longitude==-118.33000))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Lejops polygrammus" & Latitude==58.75556 & Longitude==-94.07917))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Lejops polygrammus" & Latitude==58.71667 & Longitude==-94.11667))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Microdon ruficrus (basicornis)" & Latitude==34.05223 & Longitude==-118.24375))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Parasyrphus semiinterruptus" & Latitude==63.73440 & Longitude==-148.91790))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Parhelophilus laetus" & Latitude==66.88950 & Longitude==-151.49660))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Pipiza nigripilosa" & Latitude==61.56603 & Longitude==-149.25049))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Platycheirus trichopus" & Latitude==42.73411 & Longitude==-80.69964))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Sphaerophoria sulphuripes" & Latitude==61.54194 & Longitude==-149.2297))
SyrphidDataset <- filter(SyrphidDataset, !(genusspecies=="Sphegina flavomaculata (notata)" & Latitude==46.85305 & Longitude==-121.75313))
...
#Save or load Syrphid Dataset
#save(SyrphidDataset, file="Datasets/SyrphidDataset.RData")
load("Datasets/SyrphidDataset.RData")
...
# Create/load list of species to be analyzed
#splist <- sort(unique(SyrphidDataset$genusspecies)) #CREATE
#save(splist, file="MainModel/splist.RData") #SAVE
load("MainModel/splist.RData") #LOAD
...
get.range <- function(sp){ 
  ...
  #plot(latitude_1, lwd=1.5, col=rgb(0,0,0, alpha=0.05), add=T)
  #plot(latitude_2, lwd=1.5, col=rgb(0,0,0, alpha=0.05), add=T)
  #plot(latitude_3, lwd=1.5, col=rgb(0,0,0, alpha=0.05), add=T)
  #plot(latitude_4, lwd=1.5, col=rgb(0,0,0, alpha=0.05), add=T)
...
#PLOTTING ALL RANGES (along with latitude gridlines). Make sure to set the get.range function to plot.
#longitudes <- seq(-170, -50, by = 1)  # Generate points along a latitude every 1 degree of longitude
#latitude_1 <- st_transform(st_sfc(st_linestring(as.matrix(data.frame(lon = longitudes, lat = rep(35, length(longitudes))))), crs=4326), crs=na_laea)
#latitude_2 <- st_transform(st_sfc(st_linestring(as.matrix(data.frame(lon = longitudes, lat = rep(45, length(longitudes))))), crs=4326), crs=na_laea)
#latitude_3 <- st_transform(st_sfc(st_linestring(as.matrix(data.frame(lon = longitudes, lat = rep(55, length(longitudes))))), crs=4326), crs=na_laea)
#latitude_4 <- st_transform(st_sfc(st_linestring(as.matrix(data.frame(lon = longitudes, lat = rep(65, length(longitudes))))), crs=4326), crs=na_laea)
#sapply(as.list(1:22), get.range)
...
Species_table$RangeSize <- NA
for (row in 1:nrow(Species_table)){
  Species_table$RangeSize[row] <- length(which(sp.range[row,])) 
}
...
for (j in 1:(EraSize/IntervalSize)){ 
  for (i in 1:length(EraLabels)){
    ...
    if(i == 1){
      ...
      Xj <- abind(Xj,Xij, along=3)} #Bind together the sp x site observations from all eras
  }
  if(j == 1){
    X <- Xj
  }else{
...
# Save/load the data on your computer
#save(relevant.dat,file="MainModel/relevant.dat.RData")     #SAVE
load("MainModel/relevant.dat.RData")                    #LOAD
...
################################################################################
# Written in BUGS language to be used by JAGS. Not written in R; THIS CODE IS NOT USED AND NOT MEANT TO BE RUN.
# The model is presented here for annotating only.
...
#                       n.iter=60e3,   #Number of iterations of each MCMC chain. How many samples to take.
#                       n.burnin=5e3, #Number of burn-in/warmup iterations. Reduces dependence of the chain on the starting values of MCMC sampling.
#                       n.adapt=3e3,  #These iterations are used to tune the behaviour of the MCMC sampler, making it more efficient
...
#save(model.out,file="MainModel/model.out.RData")     #SAVE
load("MainModel/model.out.RData")                    #LOAD
...
#save(ModelSummary,file="MainModel/ModelSummary.RData")  #SAVE
load("MainModel/ModelSummary.RData")                    #LOAD
...
# Plotting a single specified posterior, for diagnostics
#plot(MCMC[,"mu.psi"], main = "mu.psi") #plot the mcmc output (all samples/chains) for the desired parameter. The mcmc.list object within model.out is a list of 4 chains (each an mcmc object), each with the dimensions [#samples,#parameters]. main is the title
...
#save(OccupancySummary,file="MainModel/OccupancySummary.RData")
# Load Occupancy Summary
load("MainModel/OccupancySummary.RData")
...
# Save/Load Delta Occupancy Summary
#save(delta.occSummary,file="MainModel/delta.occSummary.RData") #SAVE
load("MainModel/delta.occSummary.RData") #LOAD
...
# List of species in recent increase (increased between the 2 most recent eras)
RecentIncrease_delta.occ <- filter(delta.occSummary, Lower95 >= 0 & Upper95 > 0 & EraContrast==paste(relevant.dat$nera,"-",relevant.dat$nera-1,sep=""))
RecentIncrease_sp <- unique(filter(RecentIncrease_delta.occ, Species!="cross.sp.mean")$Species)
...
# List of species with historical increase (increased between the 2 terminal areas)
HistoricIncrease_delta.occ <- filter(delta.occSummary, Lower95 >= 0 & Upper95 > 0 & EraContrast==paste(relevant.dat$nera,"-1",sep=""))
HistoricIncrease_sp <- unique(filter(HistoricIncrease_delta.occ, Species!="cross.sp.mean")$Species)

# List of species with weakly supported recent or historic change in occupancy
WeakSupport_sp <- splist[!(splist %in% c(RecentDecline_sp,HistoricDecline_sp,RecentIncrease_sp,HistoricIncrease_sp))]

# Create Results table with all of these lists
#max_length <- max(length(RecentDecline_sp),length(HistoricDecline_sp),length(RecentIncrease_sp),length(HistoricIncrease_sp),length(WeakSupport_sp)) #determine length of columns needed
...
#                            RecentIncrease = c(paste("Increased between", EraLabels[relevant.dat$nera-1] ,"and", EraLabels[relevant.dat$nera]),RecentIncrease_sp,rep("",max_length-length(RecentIncrease_sp))),
#                            HistoricIncrease = c(paste("Increased between", EraLabels[1] ,"and", EraLabels[relevant.dat$nera]),HistoricIncrease_sp,rep("",max_length-length(HistoricIncrease_sp))),
#                            WeakSupport = c("No significant change over recent or historical time frames", WeakSupport_sp,rep("",max_length-length(WeakSupport_sp))))

# Save or load Results table
#write.csv(Results.table,"MainModel/Results.table.csv",row.names=F) #Save results
Results.table <- read.csv("MainModel/Results.table.csv") #Load results

#### Add Recent Trend and Historical Trend columns to the Species Table ####
Species_table$RecentTrend <- NA #create empty columns to populate
Species_table$HistoricTrend <- NA
Species_table$RecentEstimate <- NA
Species_table$HistoricEstimate <-NA
Recent_delta.occ <- filter(delta.occSummary, EraContrast=="4-3")[-1,] #All delta occupancy estimates for recent change
Historic_delta.occ <- filter(delta.occSummary, EraContrast=="4-1")[-1,] #All delta occupancy estimates for historical change
# Recent Trend
for (i in 1:nrow(Species_table)){ 
  Species_table$RecentEstimate[i] <- paste(round(Recent_delta.occ$Mean[i],3)," [",round(Recent_delta.occ$Lower95[i],3),", ",round(Recent_delta.occ$Upper95[i],3),"]",sep="")
  if(Species_table$genusspecies[i] %in% RecentDecline_sp){
    Species_table$RecentTrend[i] <- "Decline"
  }else{
    if(Species_table$genusspecies[i] %in% RecentIncrease_sp){
      Species_table$RecentTrend[i] <- "Increase"
    }else{
      Species_table$RecentTrend[i] <- "WeakSupport"
    }}}
# Historical Trend
for (i in 1:nrow(Species_table)){ 
  Species_table$HistoricEstimate[i] <- paste(round(Historic_delta.occ$Mean[i],3)," [",round(Historic_delta.occ$Lower95[i],3),", ",round(Historic_delta.occ$Upper95[i],3),"]",sep="")
  if(Species_table$genusspecies[i] %in% HistoricDecline_sp){
    Species_table$HistoricTrend[i] <- "Decline"
  }else{
    if(Species_table$genusspecies[i] %in% HistoricIncrease_sp){
      Species_table$HistoricTrend[i] <- "Increase"
    }else{
      Species_table$HistoricTrend[i] <- "WeakSupport"
    }}}

# Save or load the updated Species Table
#write.csv(Species_table, file="MainModel/Species_table.csv", row.names=F)
Species_table <- read.csv("MainModel/Species_table.csv")

# Make sure columns are treated as correct variable type
Species_table$RecentTrend <- as.factor(Species_table$RecentTrend)
Species_table$RecentEstimate <- as.character(Species_table$RecentEstimate)
Species_table$HistoricTrend <- as.factor(Species_table$HistoricTrend)
Species_table$HistoricEstimate <- as.character(Species_table$HistoricEstimate)
...
######################################################
#### Association between Recent/Historical Trends ####
######################################################
...
##############################################################
#### Post-Hoc Trait Analyses (Body Size and Larval Guild) ####
##############################################################
# Note: Range size and rot-hole specialization had non-significant effects and were moved to supplementary material (further below in this script)

### Is Body Size associated with trends? t-test ###
# Create table of body sizes (for only species with strongly supported trends) taken from Skevington et al. (2019) and BugGuide.com
AllDeclines <- union(RecentDecline_sp, HistoricDecline_sp) 
AllIncrease <- union(RecentIncrease_sp, HistoricIncrease_sp)
AllDeclinesAndIncrease <- union(AllDeclines, AllIncrease) #Find all species with strongly supported trends
...
# Add bodysize column to species table
#Species_table <- right_join(BodySize_table, Species_table, by="genusspecies") %>% relocate("BodySize",.after="RangeSize")
...
# Calculate how much bigger increasing species are, on average.
#mean(filter(Species_table, is.na(BodySize)==F & RecentTrend=="Increase")$BodySize) -mean(filter(Species_table, is.na(BodySize)==F & RecentTrend=="Decline")$BodySize)
# Historical Trend
...
### Is larval guild associated with Recent Decline/Increase? Chi square test ###
...
### Figure 1: Map of records. This may take a minute.
...
legend('bottomleft', legend = factor(c("CNC","LACM","UGuelph","Spencer","UAlaska","Strickland","UTexas")), col = c("#F50E0E","#E850FF","#36CC0A","#3129F9","#00F1FF","#E6EA16","#30B1F9"), pch = 16, cex = 1.6, inset = c(0.04, 0.32), y.intersp = 0.32, x.intersp =0.2, bty='n')
#title(paste("Syrphidae (n = ",nrow(SyrphidDataset),")"),cex.main=3)
#Exported as pdf, landscape orientation, 12 x 12 inches


### Figure 2: Sampling effort histograms
# Records per year
...
p1 <- ggplot(data=SyrphidDataset, aes(x=Year)) + 
  geom_bar(width=1, fill="grey80",color="grey20", alpha=1) + 
  theme(axis.text.x=element_text(angle=0, vjust=1, hjust=0.38), panel.background=element_blank(),axis.line=element_line(color="black"),panel.grid.major.x = element_blank(),axis.title = element_text(size = 20), axis.text=element_text(size=18, colour="black"), plot.margin = margin(10, 20, 10, 10)) + 
  scale_x_discrete(limits=seq(1900,2020,30)) + 
  xlab("") + 
  ylab("Number of syrphid records") + 
  scale_y_continuous(expand = c(0, 0)) +
  annotate("text", x = -Inf, y = Inf, label = "(a)",cex=11, hjust = -0.2, vjust = 1, fontface = "bold")
# Specimens per era
...
p2 <- ggplot(data=GridID.per.year, aes(x=Year, y=grids)) + 
  ...
  scale_y_continuous(expand = c(0, 0)) +
  annotate("text", x = -Inf, y = Inf, label = "(b)",cex=11, hjust = -0.2, vjust = 1, fontface = "bold")
# Combine plots. Align y-axes using ggplotGrob()
g1 <- ggplotGrob(p1)
g2 <- ggplotGrob(p2)
...
rm(p1,p2,g1,g2,GridID.per.era,GridID.per.year)
#Exported as PDF, landscape orientation, 9 x 9 inches


### Figure 3: Caterpillar plots of recent/historical delta occupancy for all species
# Recent
caterpillar_plot_delta.occ.recent <- function(era1,era2){
  ...
  ggplot(delta.occ.to.plot[-which(delta.occ.to.plot$Species=="cross.sp.mean"),], aes(x = Species, y = Mean)) + #do not plot the cross.sp.mean as a vertical bar (take it out of the dataset)
    #Custom gridlines for Recent Change plot:
    geom_hline(yintercept = c(-1,1,2), color = "grey90", linewidth = 0.3) +
    geom_errorbar(aes(ymin = Lower95, ymax = Upper95, color=ifelse(Lower95<0 & Upper95>0,"weaksupport",ifelse(Upper95<=0,"decline","increase"))), width = 0, position = position_dodge(width = 0), linewidth=2) +
    geom_point(position = position_dodge(width = 0), size=0.75, color="black") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    #Shading for 95% CI of Recent cross species mean:
    geom_ribbon(aes(x= 1:relevant.dat$nsp, ymin = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Lower95, ymax = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Upper95), fill = "red", alpha = 0.15) + 
    ...
    #Custom labels for Recent Change plot:
    scale_y_continuous(breaks = c(-1,-0.105,0,1,2), minor_breaks = c(-0.5,0.5,1.5), labels = c("-100","-10.4","0","100","200")) + #Labels for eras 4-3
    labs(x = "", y = "Occupancy change between 1961-1990 and 1991-2020 (%)") +
    scale_color_manual(values=c("weaksupport" = "#B1B1B1", "decline" = "#AC4F55", "increase" = "#2F9BBF")) +
    guides(color=F) +
    theme_minimal() +
    geom_vline(xintercept = 0.5, color = "black") +
    theme(panel.grid.major.y = element_blank(),axis.ticks.y = element_line(color = "black"), panel.grid.major.x=element_blank(), axis.text.x=element_blank(), axis.text.y = element_text(size = 22, colour="black"), plot.title = element_text(face = "bold"), 
          axis.title.y = element_text(margin = margin(r = 10), size=25), axis.title.x = element_text(size=29)) +
    annotate("text", x = -Inf, y = Inf, label = "(a)",cex=16, hjust = -0.5, vjust = 1.5, fontface = "bold")
  }
#caterpillar_plot_delta.occ.recent(3,4)
#Historical
caterpillar_plot_delta.occ.historical <- function(era1,era2){
  contrast.to.plot <- ifelse(era1 > era2, paste(era1,"-",era2,sep=""), paste(era2,"-",era1,sep="")) #Find the name of the contrast to plot. Make sure it is in the format [larger era]-[lesser era]
  delta.occ.to.plot <- filter(delta.occSummary, EraContrast==contrast.to.plot) %>% arrange(Mean) #Filter the delta.occupancy values to only the desired era contrast. Sort by ascending Mean
  delta.occ.to.plot$Species <- factor(delta.occ.to.plot$Species, levels=unique(delta.occ.to.plot$Species),ordered=T) #Do this to prevent ggplot from automatically sorting the species on the x axis alphabetically
  #Create caterpillar plot using ggplot2
  ggplot(delta.occ.to.plot[-which(delta.occ.to.plot$Species=="cross.sp.mean"),], aes(x = Species, y = Mean)) + #do not plot the cross.sp.mean as a vertical bar (take it out of the dataset)
    #Custom gridlines for Historic Change plot:
    geom_hline(yintercept = c(-1,1,2,3), color = "grey90", linewidth = 0.3) +
    geom_errorbar(aes(ymin = Lower95, ymax = Upper95, color=ifelse(Lower95<0 & Upper95>0,"weaksupport",ifelse(Upper95<=0,"decline","increase"))), width = 0, position = position_dodge(width = 0), linewidth=2) +
    geom_point(position = position_dodge(width = 0), size=0.75, color="black") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    #Shading for 95% CI of Historical cross species mean:
    geom_ribbon(aes(x= 1:relevant.dat$nsp, ymin = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Lower95, ymax = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Upper95), fill = "black", alpha = 0.10) + 
    geom_hline(yintercept = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Mean, linetype = "dashed", color = "red2", linewidth = 0.5) + #cross-species mean
    #Custom labels for Historic Change plot:
    scale_y_continuous(breaks = c(-1,-0.01,0.1,1,2,3), minor_breaks = c(-0.5,0.5,1.5,2.5), labels = c("-100","0","8.2","100","200","300")) + #Labels for eras 4-1
    labs(x = paste("Species identity (n=",relevant.dat$nsp,")",sep=""), y = "Occupancy change between 1900-1930 and 1991-2020 (%)") +
    scale_color_manual(values=c("weaksupport" = "#B1B1B1", "decline" = "#AC4F55", "increase" = "#2F9BBF")) +
    guides(color=F) +
    theme_minimal() +
    geom_vline(xintercept = 0.5, color = "black") +
    theme(panel.grid.major.y = element_blank(),axis.ticks.y = element_line(color = "black"), panel.grid.major.x=element_blank(), axis.text.x=element_blank(), axis.text.y = element_text(size = 22, colour="black"), plot.title = element_text(face = "bold"), 
          axis.title.y = element_text(margin = margin(r = 10), size=25), axis.title.x = element_text(size=29)) +
    annotate("text", x = -Inf, y = Inf, label = "(b)",cex=16, hjust = -0.5, vjust = 1.5, fontface = "bold")
  }
#caterpillar_plot_delta.occ.historical(1,4)
## Combining the two into a single plot
grid.arrange(caterpillar_plot_delta.occ.recent(3,4),caterpillar_plot_delta.occ.historical(1,4), ncol=1)
#Exported as PDF, portrait orientation, 20 x 19 inches.


### Figure 4: Box plot of occupancy in each era across all species
boxplot_all.sp.occ <- function(){
  boxplot.occ.data <- list()
  for (era in 1:relevant.dat$nera){
    combined.posteriors <- vector(mode="numeric")
    for (sp in splist){
      combined.posteriors <- append(combined.posteriors,unlist(Occupancy_list[[sp]][era]))}
    boxplot.occ.data <- append(boxplot.occ.data,list(combined.posteriors))}
  boxplot(boxplot.occ.data,names=c("1900-1930","1931-1960","1961-1990","1991-2020"),pch=1,cex=0.5, las=1,outline=F)
  #mtext("Occupancy estimates in each era across all species", line=2, outer=F, cex=1) #add an overall title
  mtext("Occupancy", side=2, line=2.8, adj=0.5, cex=1.2)
  mtext("Era", side=1, line=2.8, adj=0.5, cex=1.2)
  mtext(text = c("a","bc","b", "ac"), side=3, at=1:4, line=0.5, cex=1.2) #Adding Tukeys
}
boxplot_all.sp.occ()
#Exported as PDF, landscape orientation, 6 x 6 inches.


### Figure 5: Plot of BodySize vs. Recent Trends
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
par(mfrow=c(1,1))
boxplot(BodySize ~ factor(filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport")$RecentTrend), filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport"), #Do not include WeakSupport column or species with no BodySize input
        xlab="Recent trend (between 1961-1990 and 1991-2020)",ylab="Body size (mm)",cex.lab=1.15,cex.axis=1.25,las=1,outline=F,ylim = range(filter(Species_table, is.na(BodySize)==F)$BodySize))
stripchart(BodySize ~ factor(filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport")$RecentTrend), filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport"), #Adding raw data points
           method="jitter", vertical=TRUE, add=TRUE, pch=16, cex=0.5,col=adjustcolor("red", alpha.f = 0.6))
mtext(text = c("Wilcoxon rank-sum W=275, p=0.006"), side=3, line=0.5, cex=1) #Adding Tukeys
#Exported as PDF, landscape orientation, 5 x 5 inches


### Figure 6: Mosaic plot of Larval Guild vs. Recent Occupancy Trend.
grid.arrange(mosaic(X2.larval.recent, shade=T, legend=F, rot_labels=c(90,90,0,0), just_labels=c("left","right"), offset_varnames = c(4.5,5), margins = c(7, 2, 2, 6), set_varnames=c(LarvalGuild="Larval feeding guild",RecentTrend="Recent trend (between 1961-1990 and 1991-2020)"),gp_labels = gpar(fontsize = 10),gp_varnames = gpar(fontsize = 11, fontface=1)))
#Exported as PDF, landscape orientation, 4.54 by 4.91 inches
...
#save(PostPredictiveChecks, file="PosteriorPredCheck/PosteriorPredictiveChecks.RData") #save results
load("PosteriorPredCheck/PosteriorPredictiveChecks.RData") #load results
View(PostPredictiveChecks[[1]]) #View a Posterior Predictive Check
mean(PostPredictiveChecks[[1]][-1,]$PercentSimilarity) #View mean percent similarity of a Posterior Predictive Check
mean(abs(PostPredictiveChecks[[1]][-1,]$DifferenceInPositiveObs)) #View mean absolute difference in number of detections


##############################
#### Sensitivity Analysis ####
##############################
# Sensitivity analysis model variants will be run in a separate script. See SensitivityModels.R
# SensitivityModels.R differs from the original model script in 3 ways: (1) either CellSize, Erasize, or IntervalSize is altered each time. (2) Only 79 species are analyzed (to reduce computational load). (3) MCMC sampling is reduced (only 30k iterations, to reduce computational load)


################################################################
#### Post-Hoc Tests: The Non-Significant Trait Correlations ####
################################################################

## Is range size correlated with Recent Decline/Increase? One way ANOVA ##
# Test assumptions:
#  -the residuals are normally distributed
#  -the error variance is the same for all groups (homoscedasticity)
#  -the residuals are independent.
aov.range.recent <- aov(RangeSize ~ RecentTrend, Species_table)
#plot(aov.range.recent)
#shapiro.test(residuals(aov.range.recent)) #Residuals are not normal
#leveneTest(RangeSize ~ RecentTrend, Species_table) #There is homoscedasticity
...
kruskal.test(RangeSize ~ RecentTrend, Species_table) #range size has significant effect on RecentTrend
...
dunnTest(RangeSize ~ RecentTrend, Species_table, method = "bonferroni")
# Species with small range sizes are more likely to have weakly supported trends. Perhaps this reflects a lack of data on species with small range sizes, which results in large confidence intervals and thus less likely to have a strongly supported trend.

# Same analysis but with Historic Trends:
aov.range.historic <- aov(RangeSize ~ HistoricTrend, Species_table)
#plot(aov.range.historic)
#shapiro.test(residuals(aov.range.historic)) #Residuals are not normal
#leveneTest(RangeSize ~ HistoricTrend, Species_table) #There is homoscedasticity
kruskal.test(RangeSize ~ HistoricTrend, Species_table) #range size has significant effect on HistoricTrend
...
# Cannot use Tukey's post hoc after a non-parametric test. Use Dunn's Test instead. Check if results are identical to Tukey's, so we may still use Tukey's results for easy plotting.
dunnTest(RangeSize ~ HistoricTrend, Species_table, method = "bonferroni")
# Species with small range sizes are also more likely to have weakly supported historic trends.


## Is being a rot-hole specialist associated with recent decline/Increase? Chi Square Test ##
# Add column to species table denoting whether a species is a rot hole specialist
Species_table$rothole <- NA
Species_table <- relocate(Species_table, "rothole",.after="BodySize")
for (i in 1:nrow(Species_table)){
...
Introduced_table <- matrix(ncol=2, byrow=TRUE, dimnames = list(NULL,c("genusspecies","introduced")),
                           c("Anasimyia anausis", "no",
                             ...
                             "Syritta pipiens", "yes",
                             "Eumerus funeralis", "yes",  #Add this apparently stable but introduced species
                             "Eumerus strigatus", "yes" #Add this apparently stable but introduced species
                           )) %>% data.frame()
# Update Species table
#Species_table <- right_join(Introduced_table, Species_table, by="genusspecies")
#Species_table <- relocate(Species_table,"introduced",.after="rothole") #add Introduced column to species table
# Of the 5 introduced species with strongly supported changes in occupancy, 4 were increasing and 1 was in decline. 2 introduced species exhibited only weakly supported change in occupancy.
# Sort and save the updated Species Table
#Species_table <- arrange(Species_table,genusspecies) #sort alphabetically
#write.csv(Species_table, file="MainModel/Species_table.csv", row.names = F)
...
### Table S1: Synonyms Table ###
#write.csv(Synonyms_table[,c(2,1,3,4)], "MainModel/SynonymsTable.csv") 


### Table S2: Results Table showing each species and their recent and historical estimates ###
#Results.supplement <- Species_table[,c("genusspecies","RecentEstimate","HistoricEstimate","RecentTrend","HistoricTrend")] %>% arrange(RecentTrend,HistoricTrend,genusspecies) #sort by recent trend, then historic trend, then alphabetically
#write.csv(Results.supplement, file="Results.supplement.csv", row.names = F)


### Figure S1: Prior Predictive Check plot is plotted in another script. See PriorPredictiveCheck.R ###


### Figures S2-S5: Simulation plots are plotted in another script. See Simulation.R ###


### Figures S6-S8: Posterior Predictive Check ###
load("PosteriorPredCheck/PosteriorPredictiveChecks.RData") #load Posterior Predictive Check results
### Figure S6: Stacked Barplot of Percent Similarity of observations for each species, for a given simulated dataset
plot.PercentSimilarity.PPC <- function(PPCnum){
  ...
  abline(a=mean(PostPredictiveChecks[[PPCnum]][-1,]$PercentSimilarity), b=0, lty=1, lwd=3, col="red") #average percent similarity
  legend(c(165,315),c(38,1),c("Simulated 1, Real 0", "Simulated 0, Real 1","Matching 1's","Matching 0's"), fill = c("#F6FEAA","#FFE479","#74C0D2","#348FA4"), cex = 2, inset = c(0.1, 0.35), y.intersp = 0.76, x.intersp =0.1)}
# Plotting
par(mar = c(5, 5, 4, 2) + 0.1) #Increase left margin so y axis title doesnt get cut off
plot.PercentSimilarity.PPC(1)
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
### Figure S7: Scatterplot of Number of simulated vs. real detections for each spp, for a given simulated dataset
...
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
### Figure S8: Repeated Posterior Predictive Checks
...
plot.PercentSimilarity.PPC <- function(PPCnum){
  ...
  StackedBars <- rbind(100*PostPredictiveCheck.plot$RightZeroes/PostPredictiveCheck.plot$NumObservations,
                       ...
                       100*PostPredictiveCheck.plot$WrongZeroes/PostPredictiveCheck.plot$NumObservations,
                       100*PostPredictiveCheck.plot$WrongOnes/PostPredictiveCheck.plot$NumObservations)
  barplot(StackedBars, ylim=c(0,100), col=c("#348FA4","#74C0D2","#FFE479","#F6FEAA"), yaxt = ifelse(PPCnum==1,"s","n"), ylab=ifelse(PPCnum==1,"Percentage of the species' dataset (%)",""), xlab= ifelse(PPCnum==2,paste("Species identity (n=",length(splist),")",sep=""),""), space=0, border="transparent", cex.lab=2.4, las=1) #Create stacked bar plot
  #axis(side = 2, at = seq(0, 100, by = 10)) #y-axis ticks go up by 10's
  abline(a=mean(PostPredictiveChecks[[PPCnum]][-1,]$PercentSimilarity), b=0, lty=1, lwd=3, col="red") #average percent similarity
...
plot.numDetections.PPC <- function(PPCnum){
  PostPredictiveCheck.plot <- arrange(PostPredictiveChecks[[PPCnum]][-1,],PositiveObs.real) #sort the PPC by number of real detections, and remove the first row that displays totals.
  plot(PostPredictiveCheck.plot$PositiveObs.real, pch=19, col="blue", cex=0.5, ylim=c(0,1000), ylab= ifelse(PPCnum==1,"Number of Detections",""), xlab=ifelse(PPCnum==2,paste("Species identity (n=",length(splist),")",sep=""),""), xaxt="n", cex.lab=2.4, las=1, yaxt="n")
...
### Figures S9-S10: Sensitivity Analysis ###
### Figure S9: Total Number of Species in Decline and Increase
# Function to plot number of species in decline and increase at all resolutions:
plot.Sensitivity.TotalDeclinesIncreases <- function(type){
  ...
  Results.table.low <- read.csv(ifelse(type=="Temporal (era)","SensitivityAnalysis/3eras/Results.table.csv",ifelse(type=="Temporal (interval)","SensitivityAnalysis/3intervals/Results.table.csv","SensitivityAnalysis/200x200km/Results.table.csv")))
  ...
  Increases.high.recent <- setdiff(Results.table.high[-1,]$RecentIncrease[Results.table.high[-1,]$RecentIncrease!=""], Results.table.high[-1,]$HistoricIncrease[Results.table.high[-1,]$HistoricIncrease!=""])
  Increases.high.historic <- setdiff(Results.table.high[-1,]$HistoricIncrease[Results.table.high[-1,]$HistoricIncrease!=""], Results.table.high[-1,]$RecentIncrease[Results.table.high[-1,]$RecentIncrease!=""])
  Increases.high.both <- intersect(Results.table.high[-1,]$RecentIncrease[Results.table.high[-1,]$RecentIncrease!=""], Results.table.high[-1,]$HistoricIncrease[Results.table.high[-1,]$HistoricIncrease!=""])
  Increases.medium.recent <- setdiff(Results.table.medium[-1,]$RecentIncrease[Results.table.medium[-1,]$RecentIncrease!=""], Results.table.medium[-1,]$HistoricIncrease[Results.table.medium[-1,]$HistoricIncrease!=""])
  Increases.medium.historic <- setdiff(Results.table.medium[-1,]$HistoricIncrease[Results.table.medium[-1,]$HistoricIncrease!=""], Results.table.medium[-1,]$RecentIncrease[Results.table.medium[-1,]$RecentIncrease!=""])
  Increases.medium.both <- intersect(Results.table.medium[-1,]$RecentIncrease[Results.table.medium[-1,]$RecentIncrease!=""], Results.table.medium[-1,]$HistoricIncrease[Results.table.medium[-1,]$HistoricIncrease!=""])
  Increases.low.recent <- setdiff(Results.table.low[-1,]$RecentIncrease[Results.table.low[-1,]$RecentIncrease!=""], Results.table.low[-1,]$HistoricIncrease[Results.table.low[-1,]$HistoricIncrease!=""])
  Increases.low.historic <- setdiff(Results.table.low[-1,]$HistoricIncrease[Results.table.low[-1,]$HistoricIncrease!=""], Results.table.low[-1,]$RecentIncrease[Results.table.low[-1,]$RecentIncrease!=""])
  Increases.low.both <- intersect(Results.table.low[-1,]$RecentIncrease[Results.table.low[-1,]$RecentIncrease!=""], Results.table.low[-1,]$HistoricIncrease[Results.table.low[-1,]$HistoricIncrease!=""])
  # Create labels for each resolution
  if(type=="Temporal (era)"){
    ResolutionLabels <- c("20","30","40")
    Xlabel <- "Era size (yrs)"
    Ylabel <- ""
  }else{
  ...
  sensitivity.data <- data.frame(
    ...
    Bar = rep(c("Decline", "Increase"), times = 3, each = 3),
    Stack = rep(c("Recent","Historic","Both"), times = 6),
    Value = c(length(Declines.high.recent), length(Declines.high.historic), length(Declines.high.both), length(Increases.high.recent), length(Increases.high.historic), length(Increases.high.both),
              length(Declines.medium.recent), length(Declines.medium.historic),length(Declines.medium.both), length(Increases.medium.recent),length(Increases.medium.historic), length(Increases.medium.both),
              length(Declines.low.recent),length(Declines.low.historic), length(Declines.low.both), length(Increases.low.recent),length(Increases.low.historic),length(Increases.low.both)))
  ...
  sensitivity.data$BarStack <- factor(sensitivity.data$BarStack, levels = c("Decline.Recent","Decline.Historic","Decline.Both","Increase.Recent","Increase.Historic","Increase.Both"))
  ...
  ggplot(sensitivity.data, aes(x = Bar, y = Value, fill = BarStack)) +
    ...
    scale_fill_manual(values= c(
      ...
      "Increase.Both" = "#216C86",
      "Increase.Historic" = "#2F9BBF",
      "Increase.Recent" = "#3CBCE6"), 
      labels = c(
        ...
        "Increase.Both" = "Both Recent and Historical Increase",
        "Increase.Historic" = "Historical Increase only",
        "Increase.Recent" = "Recent Increase only")) +
    if(type=="Temporal (interval)"){ #for multi panel plots. Only keep legend for the last plot.
      theme(legend.position = c(0.5,0.75), legend.text = element_text(size = 18), legend.background = element_rect(fill = "white", color = NA), strip.text = element_text(size = 22, margin = margin(t = -1.8)), title = element_blank(), axis.title.x = element_text(size=24,margin = margin(t = 25)), axis.title.y = element_text(size=22, margin = margin(r = 10)), axis.text.y=element_blank() , axis.text.x = element_blank(), panel.grid.major.x=element_blank(), panel.spacing = unit(0, "lines")) 
    }else{
      if(type=="Temporal (era)"){
...
grid.arrange(plot.Sensitivity.TotalDeclinesIncreases("Spatial"),plot.Sensitivity.TotalDeclinesIncreases("Temporal (era)"),plot.Sensitivity.TotalDeclinesIncreases("Temporal (interval)"),ncol=3)
### Figure S10: Estimates of Across-Species Occupancy Change
# Function to plot the recent and historical changes in across-species occupancy at all resolutions
plot.Sensitivity.DeltaOcc <- function(type){
  #Function to find the delta occupancy between 2 given eras for a given model variant
  compute_sensitivity_delta_occ <- function(type,resolution,baseline){
    #Load in correct model output based on given arguments
    if(resolution=="high"){
      if(type=="Temporal (era)"){
        load("SensitivityAnalysis/6eras/model.out.RData")
        reso.label<-"20"
      }else{
        if(type=="Temporal (interval)"){
          load("SensitivityAnalysis/10intervals/model.out.RData")
          reso.label<-"3"
        }else{
          load("SensitivityAnalysis/50x50km/model.out.RData")
          reso.label<-"50x50"}}
    }else{
      if(resolution=="medium"){
        if(type=="Temporal (era)"){
          load("SensitivityAnalysis/BaseModel/model.out.RData")
          reso.label<-"30"
        }else{
          if(type=="Temporal (interval)"){
            load("SensitivityAnalysis/BaseModel/model.out.RData")
        ...
        if(type=="Temporal (era)"){
          load("SensitivityAnalysis/3eras/model.out.RData")
          reso.label<-"40"
        }else{
          if(type=="Temporal (interval)"){
            load("SensitivityAnalysis/3intervals/model.out.RData")
            reso.label<-"10"
          }else{
            load("SensitivityAnalysis/200x200km/model.out.RData")
            reso.label<-"200x200"}}
      }}
    MCMC <- model.out$jags.out$mcmc #use MCMC as shorthand for quick/easy access to the mcmc samples in the model output
    #Compute cross-species occupancy in the given eras
    era1 <- model.out$jags.data$nera
    era2 <- ifelse(baseline=="recent",model.out$jags.data$nera-1,1)
    #Obtain occupancy of era1 (i.e., era 4)
    mu.psi <- matrix(as.matrix(MCMC[,"mu.psi"]), nrow=model.out$jags.out$sample, ncol=length(MCMC)) #Obtain mu.psi distribution
    mu.psi.era <- matrix(as.matrix(MCMC[,paste("mu.psi.era[",era1,"]",sep="")]), nrow=model.out$jags.out$sample, ncol=length(MCMC))
    era1.occ <- as.matrix(plogis(as.matrix(mu.psi+mu.psi.era))) #Compute occupancy
    if (era2==1){ #If era2 is 1, across species occupancy is simply the expit of mu.psi. The effect of era 1 (the "default" era) is already contained in mu.psi.
      mu.psi.era <- 0
    }else{ #If era is not 1, must obtain the mu.psi.era parameter (which represents the mean effect of this era across species)
      mu.psi.era <- matrix(as.matrix(MCMC[,paste("mu.psi.era[",era2,"]",sep="")]), nrow=model.out$jags.out$sample, ncol=length(MCMC))}
    era2.occ <- as.matrix(plogis(as.matrix(mu.psi+mu.psi.era))) #Compute occupancy
    #Compute delta occupancy
    delta.occ <- as.matrix((era1.occ-era2.occ)/era2.occ) #subtract the posterior distributions and divide by earlier era
    delta.occ <- as.mcmc.list(lapply(seq_len(ncol(delta.occ)), function(i) as.mcmc(delta.occ[,i]))) #turn back into mcmc object
    #Get summary stats
    delta.occ.summary <- c(type, reso.label, baseline, HPDinterval(as.mcmc(unlist(delta.occ)), prob=0.95)[[1]] , summary(delta.occ)$statistics[[1]], HPDinterval(as.mcmc(unlist(delta.occ)), prob=0.95)[[2]] , summary(delta.occ)$statistics[[2]])
    names(delta.occ.summary) <- c("type","resolution","baseline","Lower95","Mean","Upper95","SD")
    return(delta.occ.summary)
  }
  #Create table holding estimates of occupancy change for all models/baselines
  params.to.plot <- as.data.frame(rbind(compute_sensitivity_delta_occ(type,"high","recent"),
                                        compute_sensitivity_delta_occ(type,"high","historic"),
                                        compute_sensitivity_delta_occ(type,"medium","recent"),
                                        compute_sensitivity_delta_occ(type,"medium","historic"),
                                        compute_sensitivity_delta_occ(type,"low","recent"),
                                        compute_sensitivity_delta_occ(type,"low","historic")))
  ...
  params.to.plot$Lower95 <- as.numeric(params.to.plot$Lower95)
  params.to.plot$Mean <- as.numeric(params.to.plot$Mean)
  params.to.plot$Upper95 <- as.numeric(params.to.plot$Upper95)
  ytitle <- ifelse(type=="Spatial","Change in occupancy (%)","") #Only give y axis title to first plot when making multi-panel plot
  ggplot(params.to.plot, aes(x= resolution, y = Mean, color = baseline)) +
    geom_errorbar(aes(ymin = Lower95, ymax = Upper95), width = 0.25, position = position_dodge(width = 0.2), linewidth=0.9) +
    geom_point(position = position_dodge(width = 0.2), size=4) +
    scale_color_manual(values= c("historic"="#2F9BBF", "recent"="#E65F67"), labels = c(
      "recent" = "Recent baseline",
      "historic" = "Historical baseline")) +
    labs(x = ifelse(type=="Temporal (era)","Era size (yrs)",ifelse(type=="Temporal (interval)","Interval size (yrs)","Grid size (km)")), y = ytitle) +
    scale_y_continuous(labels = label_percent(), limits = c(-0.4, 0.8)) +
    theme_minimal() +
    if(type=="Spatial"){
      theme(legend.position = "none",legend.background = element_rect(fill = "white", color = NA), strip.text = element_text(size = 20, margin = margin(t = -1.8)), title = element_blank(), axis.title.x = element_text(size=24,margin = margin(t = 25)), axis.title.y = element_text(size=22, margin = margin(r = 10)), axis.text.y=element_text(size=24), axis.text.x = element_text(size=19) ,panel.grid.major.x=element_blank(), panel.spacing = unit(0, "lines")) 
    }else{
      if(type=="Temporal (interval)"){ #for multi panel plots. Only keep legend for the last plot.
        theme(legend.position = c(0.6,0.85), legend.text = element_text(size = 18), legend.background = element_rect(fill = "white", color = NA), strip.text = element_text(size = 22, margin = margin(t = -1.8)), title = element_blank(), axis.title.x = element_text(size=24,margin = margin(t = 25)), axis.title.y = element_text(size=22, margin = margin(r = 10)), axis.text.y=element_blank() , axis.text.x = element_text(size=20), panel.grid.major.x=element_blank(), panel.spacing = unit(0, "lines")) 
      }else{
        if(type=="Temporal (era)"){
          theme(legend.position = "none",legend.background = element_rect(fill = "white", color = NA), strip.text = element_text(size = 22, margin = margin(t = -1.8)), title = element_blank(), axis.title.x = element_text(size=24,margin = margin(t = 25)), axis.title.y = element_text(size=22, margin = margin(r = 10)), axis.text.y=element_blank(), axis.text.x = element_text(size=20) , panel.grid.major.x=element_blank(), panel.spacing = unit(0, "lines"))
        }}}
}
grid.arrange(plot.Sensitivity.DeltaOcc("Spatial"),plot.Sensitivity.DeltaOcc("Temporal (era)"),plot.Sensitivity.DeltaOcc("Temporal (interval)"),ncol=3)


### Figure S11: All Pairwise Contrasts Between Eras ###
caterpillar_plot_delta.occ_all <- function(era1,era2,barcolour){
  contrast.to.plot <- ifelse(era1 > era2, paste(era1,"-",era2,sep=""), paste(era2,"-",era1,sep="")) #Find the name of the contrast to plot. Make sure it is in the format [larger era]-[lesser era]
  delta.occ.to.plot <- filter(delta.occSummary, EraContrast==contrast.to.plot) %>% arrange(Mean) #Filter the delta.occupancy values to only the desired era contrast. Sort by ascending Mean
  delta.occ.to.plot$Species <- factor(delta.occ.to.plot$Species, levels=unique(delta.occ.to.plot$Species),ordered=T) #Do this to prevent ggplot from automatically sorting the species on the x axis alphabetically
  #Create caterpillar plot using ggplot2
  ggplot(delta.occ.to.plot[-which(delta.occ.to.plot$Species=="cross.sp.mean"),], aes(x = Species, y = Mean)) + #do not plot the cross.sp.mean as a vertical bar (take it out of the dataset)
    geom_errorbar(aes(ymin = Lower95, ymax = Upper95, color=ifelse(Lower95<0 & Upper95>0,"weaksupport",ifelse(Upper95<=0,"decline","increase"))), width = 0, position = position_dodge(width = 0), linewidth=0.8) +
    geom_point(position = position_dodge(width = 0), size=0.2, color="black") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    geom_ribbon(aes(x= 1:relevant.dat$nsp, ymin = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Lower95, ymax = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Upper95), fill = barcolour, alpha = 0.3) + #Shading for 95% CI of cross species mean
    geom_hline(yintercept = delta.occ.to.plot[which(delta.occ.to.plot$Species=="cross.sp.mean"),]$Mean, linetype = "dashed", color = "red2", linewidth = 0.5) + #cross-species mean
...
spacer <- nullGrob()
plots <- grid.arrange(caterpillar_plot_delta.occ_all(3,4,"red"),caterpillar_plot_delta.occ_all(2,4,"black"),spacer,spacer,caterpillar_plot_delta.occ_all(1,4,"black"),caterpillar_plot_delta.occ_all(2,3,"black"),spacer,spacer,caterpillar_plot_delta.occ_all(1,3,"#26b9ff"),caterpillar_plot_delta.occ_all(1,2,"#26b9ff"),ncol=2, heights=c(1,0.08,1,0.08,1), widths=c(1,0.88))
x_axis_label <- textGrob("Species identity (n=318)", rot = 0, vjust = 0.6, hjust = 0.5, gp = gpar(fontsize = 18))
grid.arrange(plots, x_axis_label, ncol = 1, heights = c(5, 0.2))
#Exported as PDF, landscape orientation, 14 x 10 inches. Panel labels added manually

### Figure S12: Plot of RangeSize for recent and historical trends
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
par(mfrow=c(1,2))
boxplot(RangeSize ~ RecentTrend, Species_table,
        xlab="Recent trend (between 1961-1990 and 1991-2020)",ylab="Range size (number of grid cells)",cex.lab=1.2,cex.axis=1.25)
stripchart(RangeSize ~ RecentTrend, Species_table, #Adding raw data points
           method="jitter", vertical=TRUE, add=TRUE, pch=16, cex=0.4,col=adjustcolor("red", alpha.f = 0.6))
mtext(text = c("a","a","b"), side=3, at=1:3, line=0.5, cex=1.2) #Adding Tukeys
mtext("(a)", side=3, line=2, adj=-0.12, cex=2,font=2)
boxplot(RangeSize ~ HistoricTrend, Species_table,
        xlab="Historical trend (between 1900-1930 and 1991-2020)",ylab="",cex.lab=1.2,cex.axis=1.25)
stripchart(RangeSize ~ HistoricTrend, Species_table, #Adding raw data points
           method="jitter", vertical=TRUE, add=TRUE, pch=16, cex=0.4, col=adjustcolor("red", alpha.f = 0.6))
mtext(text = c("a","a","b"), side=3, at=1:3, line=0.5, cex=1.2) #Adding Tukeys
mtext("(b)", side=3, line=2, adj=-0.12, cex=2,font=2)
#Exported as PDF, landscape orientation, 11 x 6 inches


### Figure S13: Plot of BodySize for recent AND historical trends
par(mar = c(5, 4, 4, 2) + 0.1)  # Default margin values
par(mfrow=c(1,2))
boxplot(BodySize ~ factor(filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport")$RecentTrend), filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport"), #Do not include WeakSupport column or species with no BodySize input
        xlab="Recent trend (between 1961-1990 and 1991-2020)",ylab="Body size (mm)",cex.lab=1.3,cex.axis=1.25,las=1,outline=F,ylim = range(filter(Species_table, is.na(BodySize)==F)$BodySize))
stripchart(BodySize ~ factor(filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport")$RecentTrend), filter(Species_table, is.na(BodySize)==F & RecentTrend!="WeakSupport"), #Adding raw data points
           method="jitter", vertical=TRUE, add=TRUE, pch=16, cex=0.5,col=adjustcolor("red", alpha.f = 0.6))
mtext(text = c("a","b"), side=3, at=1:2, line=0.5, cex=1.4) #Adding Tukeys
mtext("(a)", side=3, line=2, adj=-0.12, cex=2,font=2)
boxplot(BodySize ~ factor(filter(Species_table, is.na(BodySize)==F & HistoricTrend!="WeakSupport")$HistoricTrend), filter(Species_table, is.na(BodySize)==F & HistoricTrend!="WeakSupport"),
        xlab="Historical trend (between 1900-1930 and 1991-2020)",ylab="",cex.lab=1.3,cex.axis=1.25,las=1,outline=F,ylim = range(filter(Species_table, is.na(BodySize)==F)$BodySize))
stripchart(BodySize ~ factor(filter(Species_table, is.na(BodySize)==F & HistoricTrend!="WeakSupport")$HistoricTrend), filter(Species_table, is.na(BodySize)==F & HistoricTrend!="WeakSupport"), #Adding raw data points
           method="jitter", vertical=TRUE, add=TRUE, pch=16, cex=0.5, col=adjustcolor("red", alpha.f = 0.6))
mtext(text = c("a","a"), side=3, at=1:2, line=0.5, cex=1.4) #Adding Tukeys
mtext("(b)", side=3, line=2, adj=-0.12, cex=2,font=2)
#Exported as PDF, landscape orientation, 11.5 x 6 inches


### Figure S14: Mosaic plots of Larval Guild vs. Occupancy Trend (Recent AND Historical).
grid.arrange(arrangeGrob(grid.grabExpr(mosaic(X2.larval.recent, shade=T, legend=F, rot_labels=c(90,90,0,0), just_labels=c("left","right"), offset_varnames = c(5,5.5), margins = c(9, 2, 4, 11), set_varnames=c(LarvalGuild="Larval feeding guild",RecentTrend="Recent trend (between 1961-1990 and 1991-2020)"),gp_labels = gpar(fontsize = 9),gp_varnames = gpar(fontsize = 10, fontface=2))),top = textGrob("(a)", x = 0, hjust = -5.2, vjust=4, gp = gpar(fontsize = 16, fontface = "bold"))), 
             arrangeGrob(grid.grabExpr(mosaic(X2.larval.historic, shade=T, legend=F, rot_labels=c(90,90,0,0), just_labels=c("left","right"), offset_varnames = c(5,6), margins = c(9, 4, 4, 2), set_varnames=c(LarvalGuild="",HistoricTrend="Historical trend (between 1900-1930 and 1991-2020)"),gp_labels = gpar(fontsize = 9),gp_varnames = gpar(fontsize = 10, fontface=2) )),top = textGrob("(b)", x = 0, hjust = -0.8, vjust=4, gp = gpar(fontsize = 16, fontface = "bold"))), ncol=2, respect=T)
#Exported as PDF, landscape orientation, 10 x 6 inches


### Figures S15-S18: Strongly Supported Declines and Increases ###
# (Caterpillar plots of just the species in decline or just the species in increase)
# Function to italicize species names
xlab_func <- function(sp){ sapply(sp, function(label){ bquote(italic(.(label))) })}
...
caterpillar_plot_delta.occ_RecentDecline <- function(){
  ...
  ggplot(delta.occ.to.plot, aes(x = Species, y = Mean)) +
    geom_errorbar(aes(ymin = Lower95, ymax = Upper95, color=ifelse(Lower95<0 & Upper95>0,"weaksupport",ifelse(Upper95<=0,"decline","increase"))), width = 0, position = position_dodge(width = 0), linewidth=5) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    ...
    geom_point(position = position_dodge(width = 0), size=1.5, color="black") +
    coord_cartesian(ylim=c(-1,0)) + #Set y axis
    scale_y_continuous(labels = label_percent()) +
    scale_x_discrete( labels= xlab_func) +
    labs(y = "Occupancy change between 1961-1990 and 1991-2020 (%)") +
    scale_color_manual(values=c("weaksupport" = "#B1B1B180", "decline" = "#AC4F5580", "increase" = "#2F9BBF80")) +
    guides(color=F) +
    theme_minimal() +
...
#Exported as PDF, landscape orientation, 12 x 8 inches

# Recent Increases
caterpillar_plot_delta.occ_RecentIncrease <- function(){
  delta.occ.to.plot <- filter(delta.occSummary, EraContrast==paste(relevant.dat$nera,"-",relevant.dat$nera-1,sep="") & Species %in% RecentIncrease_sp) %>% arrange(Mean) #Filter the delta.occupancy values to only the desired era contrast and species in recent decline. Sort by ascending Mean
  delta.occ.to.plot$Species <- factor(delta.occ.to.plot$Species, levels=unique(delta.occ.to.plot$Species),ordered=T) #Do this to prevent ggplot from automatically sorting the species on the x axis alphabetically
  sum <- 0
  for(sp in delta.occ.to.plot$Species){ #Add all species' delta occupancy posteriors together
    sum <- matrix(as.matrix(delta.occ_list[[sp]][["4-3"]]), nrow=model.out$jags.out$sample, ncol=length(MCMC)) + sum}
  ...
  ggplot(delta.occ.to.plot, aes(x = Species, y = Mean)) +
    geom_errorbar(aes(ymin = Lower95, ymax = Upper95, color=ifelse(Lower95<0 & Upper95>0,"weaksupport",ifelse(Upper95<=0,"decline","increase"))), width = 0, position = position_dodge(width = 0), linewidth=5) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    geom_ribbon(aes(x= 1:length(Species), ymin = cross.sp.Lower95, ymax = cross.sp.Upper95), fill = "#26b9ff", alpha = 0.25) + #Shading for 95% CI of cross species mean
    geom_hline(yintercept = cross.sp.Mean, linetype = "dashed", color = "red2", linewidth = 0.5) + #cross-species mean
    geom_point(position = position_dodge(width = 0), size=1.5, color="black") +
...
caterpillar_plot_delta.occ_RecentIncrease()
#Exported as PDF, portrait orientation, 4 x 6.3 inches
...
caterpillar_plot_delta.occ_HistoricDecline <- function(){
  ...
  ggplot(delta.occ.to.plot, aes(x = Species, y = Mean)) +
    ...
    scale_y_continuous(labels = label_percent()) +
    scale_x_discrete( labels= xlab_func) +
    labs(y = "Occupancy change between 1900-1930 and 1991-2020 (%)") +
    scale_color_manual(values=c("weaksupport" = "#B1B1B180", "decline" = "#AC4F5580", "increase" = "#2F9BBF80")) +
    guides(color=F) +
    theme_minimal() +
    theme(panel.grid.major.x=element_blank(), axis.text.x = element_text(angle=90, hjust=1, vjust = 0.5, size=12), axis.text.y = element_text(size = 18), axis.title.y = element_text(margin = margin(r = 10), size=20, hjust=1), axis.title.x = element_text(size=20))}
caterpillar_plot_delta.occ_HistoricDecline()
#Exported as PDF, landscape orientation, 8.1 x 8 inches

# Historical Increases
caterpillar_plot_delta.occ_HistoricIncrease <- function(){
  delta.occ.to.plot <- filter(delta.occSummary, EraContrast==paste(relevant.dat$nera,"-",1,sep="") & Species %in% HistoricIncrease_sp) %>% arrange(Mean) #Filter the delta.occupancy values to only the desired era contrast and species in recent decline. Sort by ascending Mean
  delta.occ.to.plot$Species <- factor(delta.occ.to.plot$Species, levels=unique(delta.occ.to.plot$Species),ordered=T) #Do this to prevent ggplot from automatically sorting the species on the x axis alphabetically
  sum <- 0
  for(sp in delta.occ.to.plot$Species){ #Add all species' delta occupancy posteriors together
    sum <- matrix(as.matrix(delta.occ_list[[sp]][["4-1"]]), nrow=model.out$jags.out$sample, ncol=length(MCMC)) + sum}
  ...
  ggplot(delta.occ.to.plot, aes(x = Species, y = Mean)) +
    ...
    theme_minimal() +
    theme(panel.grid.major.x=element_blank(), axis.text.x = element_text(angle=90, hjust=1,vjust = 0.5, size=12), axis.text.y = element_text(size = 18), axis.title.y = element_text(margin = margin(r = 10), size=20, hjust=1), axis.title.x = element_text(size=20))}
caterpillar_plot_delta.occ_HistoricIncrease()
#Exported to PDF, portrait orientation, 6.8 x 7.8 inches


### Unused figure: Bar graphs of number of species in decline, increase, or none
p1 <- ggplot(Species_table, aes(x = RecentTrend, fill=RecentTrend)) +
  ...
  scale_fill_manual(values = c("Decline" = "#AC4F55", "Increase" = "#2F9BBF", "WeakSupport" = "#B1B1B1")) +
  ...
  scale_x_discrete(labels=c("Decline","Increase","Weak\nSupport"))
...
### Unused Figure: Mosaic plot of Rothole Specialists vs. Occupancy Trend
