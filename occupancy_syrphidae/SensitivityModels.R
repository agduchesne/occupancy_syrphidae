...
# Thin the species list to reduce computational load. Sort species list by number of records, then keep every 4th species.
...
Species_table <- Species_table[order(Species_table$sprecords, decreasing=TRUE),] #Sort by number of records
Species_table <- Species_table[seq(4,nrow(Species_table),by=4),] #keep every 4th species

# Filter records to only the select species.
SyrphidDataset <- filter(SyrphidDataset, genusspecies %in% Species_table$genusspecies)
...
# List of species with historic increase (increased between the 2 terminal areas)
...
max_length <- max(length(RecentDecline_sp),length(HistoricDecline_sp),length(RecentIncrease_sp),length(HistoricIncrease_sp),length(WeakSupport_sp))
