...
#write.csv(Synonyms_table[,c(2,1,3,4)], "SynonymsTable.csv") #Save synonyms table
...
#model{
  ...
  # Latent Z state for each site. Z = true presence of the species. Either a 0 or 1 and can only change between eras. Bernoulli trial with prob. of success equal to psi[sp,era] (which was defined above)
  #for(sp in 1:nsp) {
  #  for(site in 1:nsite) {
  #    for(era in 1:nera) {
  ...
  # Model the observations. Z is true (latent) occupancy state, p is prob of detection, and X is the observations
  #for(ind in 1:nind) {
  #  p.eff[ind] <-
...
#write.csv(Results.table,"Results.table.csv",row.names=F) #Save results
#Results.table <- read.csv("Results.table.csv") #Load results
...
save(PriorCheck,file="syrphid_occupancy/PriorPredCheck/PriorCheck.RData")
...
#save(PostPredictiveChecks, file="PostPredictiveChecks.RData") #save results
load("syrphid_occupancy/PostPredictiveChecks.RData") #load results
...
plot.Sensitivity.TotalDeclinesGrowths <- function(type){
  Results.table.high <- read.csv(ifelse(type=="Temporal (era)","SensitivityAnalysis/6eras/Results.table.csv",ifelse(type=="Temporal (interval)","SensitivityAnalysis/10intervals/Results.table.csv","SensitivityAnalysis/50x50km/Results.table.csv"))) #Obtain results tables at different spatial resolutions
...
#percents <- round(slices/sum(slices)*100, digits=1)
#slice_labels <- paste(c("CNC", "LACM", "UGuelph", "Spencer", "UAlaska", "Strickland", "Texas"))
#slice_labels <- paste(c("CNC", "LACM", "UGuelph", "Spencer", "UAlaska", "Strickland", "UTexas"), paste(percents,"%",sep=""), paste("(n=",slices,")",sep="")) #alternate labels
#pie(slices, slice_labels, col=c("#F50E0E70","#E850FF70","#36CC0A70","#3129F970","#00F1FF70","#E6EA1670","#30B1F970"))
...
#barplot(slices, names.arg=slice_labels) #barplot instead of pie chart
...
# Prior Predictive Check plot is plotted in the PriorCheck script


# Simulation plots are plotted in the Simulation script
...
load("FinalModel/PostPredictiveChecks.RData") #load Posterior Predictive Check results
